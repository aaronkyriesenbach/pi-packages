 ...
 */
export function applyOverrides(settings: Settings, overrides: Map<string, boolean>): Settings {
  if (overrides.size === 0) return settings;
  const packages = [...(settings.packages ?? [])];
  for (const [source, enabled] of overrides) {
    const idx = packages.findIndex((e) => (typeof e === 'string' ? e : e.source) === source);
    if (idx === -1) continue;
    if (enabled) {
      packages[idx] = source;
    } else {
