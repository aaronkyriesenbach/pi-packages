...
export class PackageListComponent {
  private packages: (PackageInfo & { _persistedEnabled?: boolean })[];
  ...
  buildViewModel(): PackageListViewModel {
    return {
      rows: this.packages.map((pkg, i) => ({
        ...
        version: pkg.version,
        enabled: pkg.enabled,
        hasPending: pkg._persistedEnabled !== undefined && pkg._persistedEnabled !== pkg.enabled,
  ...
  handleInput(data: string): void {
    if (matchesKey(data, 'escape')) {
      const hasPending = this.packages.some(
        (pkg) => pkg._persistedEnabled !== undefined && pkg._persistedEnabled !== pkg.enabled,
      );
      this.onClose({
    ...
    if (matchesKey(data, 'k') || matchesKey(data, 'up')) {
      if (this.packages.length === 0) return;
      this.selectedIndex = (this.selectedIndex - 1 + this.packages.length) % this.packages.length;
      this.invalidate();
      return;
    }
  ...
  private persistState(): void {
    for (const pkg of this.packages) {
      const entry = (this.settings.packages ?? []).find((e) =>
        typeof e === 'string' ? e === pkg.source : e.source === pkg.source,
      );
      if (!entry) continue;
      const isCurrentlyEnabled = typeof entry === 'string' || !isAllResourcesEmpty(entry);
  ...
  private rowForPkg(index: number, pkg: PackageInfo & { _persistedEnabled?: boolean }): string {
    ...
    const hasPending = pkg._persistedEnabled !== undefined && pkg._persistedEnabled !== pkg.enabled;

    const cursor = isSelected ? th.fg('accent', '>') : ' ';
    const status = pkg.enabled ? th.fg('success', '● enabled') : th.fg('dim', '○ disabled');
