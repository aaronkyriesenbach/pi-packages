...
describe('handlePackagesCommand', () => {
  ...
  it('notifies error in non-TUI mode', async () => {
    ...
    expect(notify).toHaveBeenCalledWith('/packages requires interactive mode', 'error');
  ...
  it('persists session overrides and reloads when TUI adds a new override', async () => {
    ...
    expect(deps.appendEntry).toHaveBeenCalledWith('pi-package-manager-overrides', {
      'npm:pi-lens': false,
    });
  ...
  it('resolves entries when settings.packages is undefined', async () => {
    mockReadSettings.mockResolvedValue({});
    mockReadAutoUpdateConfig.mockResolvedValue({ ...defaultAutoUpdateConfig });
  ...
  it('discards session overrides on cancel and does not reload when auto-update is unchanged', async () => {
    mockReadSettings.mockResolvedValue({ packages: ['npm:pi-lens'] });
    mockReadAutoUpdateConfig.mockResolvedValue({ ...defaultAutoUpdateConfig });

    const { ctx } = makeCtx();
    const overrides = new Map<string, boolean>();
    overrides.set('npm:existing', true);
    const deps = { appendEntry: vi.fn(), reload: vi.fn() };
    ...
    mockPackageListCtor.mockImplementation(
      (
        ...
        // Simulate a toggle made during the TUI session, then ESC to cancel.
        overrides_.set('npm:pi-lens', false);
        onClose({ settings: {}, autoUpdateEnabled: true, discarded: true });
        return { buildViewModel: vi.fn(), handleInput: vi.fn() };
      },
    );
    ...
    // Reverted to the pre-TUI override state
    expect(overrides.get('npm:existing')).toBe(true);
    expect(overrides.has('npm:pi-lens')).toBe(false);
    expect(mockWriteAutoUpdateConfig).toHaveBeenCalled();
    expect(deps.reload).not.toHaveBeenCalled();
  });

  it('reloads on cancel when auto-update toggle changed', async () => {
    mockReadSettings.mockResolvedValue({ packages: ['npm:pi-lens'] });
    mockReadAutoUpdateConfig.mockResolvedValue({ ...defaultAutoUpdateConfig, enabled: true });
    ...
    mockPackageListCtor.mockImplementation(
      (
        ...
        onClose({ settings: {}, autoUpdateEnabled: false, discarded: true });
        return { buildViewModel: vi.fn(), handleInput: vi.fn() };
      },
    );
  ...
  it('swallows backup failures and still applies effective settings', async () => {
    mockReadSettings.mockResolvedValue({ packages: ['npm:pi-lens'] });
    mockReadAutoUpdateConfig.mockResolvedValue({ ...defaultAutoUpdateConfig });
    mockBackupOriginalSettings.mockRejectedValue(new Error('disk full'));
    ...
    await expect(handlePackagesCommand(ctx, overrides, deps)).resolves.toBeUndefined();

    expect(mockBackupOriginalSettings).toHaveBeenCalled();
    expect(mockWriteSettings).toHaveBeenCalled();
  });

  it('appends an empty overrides entry when all overrides are resolved', async () => {
    mockReadSettings.mockResolvedValue({ packages: ['npm:pi-lens'] });
    mockReadAutoUpdateConfig.mockResolvedValue({ ...defaultAutoUpdateConfig });

    const { ctx } = makeCtx();
    const overrides = new Map<string, boolean>();
    overrides.set('npm:pi-lens', false); // starts with one override
    const deps = { appendEntry: vi.fn(), reload: vi.fn() };
    ...
    mockPackageListCtor.mockImplementation(
      (
        ...
        // Simulate the user persisting the toggle, clearing the session override
        overrides_.clear();
        onClose({ settings, autoUpdateEnabled });
        return { buildViewModel: vi.fn(), handleInput: vi.fn() };
      },
