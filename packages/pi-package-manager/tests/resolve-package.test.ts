...
const readPackageJson = (name: string): Promise<PackageJson | null> => {
...
describe('resolvePackageEntry', () => {
  it('resolves a string entry with full name, version, and resources', async () => {
    const result = await resolvePackageEntry('npm:pi-lens', new Map(), readPackageJson);
  ...
  it('returns null for non-npm: source', async () => {
    const result = await resolvePackageEntry('/some/path', new Map(), readPackageJson);
    expect(result).toBeNull();
  });

  it('returns version unknown and empty resources for missing package.json', async () => {
    const result = await resolvePackageEntry('npm:missing-pkg', new Map(), readPackageJson);
    expect(result).not.toBeNull();
    expect(result).toMatchObject({
      name: 'missing-pkg',
      version: 'unknown',
  ...
  it('resolves a PackageFilter entry with selective filtering', async () => {
    ...
    const result = await resolvePackageEntry(filter, new Map(), readPackageJson);
  ...
  it('applies session override to enable a disabled package', async () => {
    ...
    const result = await resolvePackageEntry(filter, overrides, readPackageJson);
  ...
  it('applies session override to disable an enabled package', async () => {
    ...
    const result = await resolvePackageEntry('npm:pi-lens', overrides, readPackageJson);
