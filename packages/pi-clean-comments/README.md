...
| Key                       | Default   | Meaning                                                                          |
| ------------------------- | --------- | -------------------------------------------------------------------------------- |
| `enforcement`             | `"nudge"` | `"nudge"` appends a reminder only; `"gate"` also strips comments over threshold. |
| `threshold`               | `2`       | Max comment-block line count `"gate"` allows before stripping it.                |
| `allowAgentBypassRequest` | `false`   | See below.                                                                       |
